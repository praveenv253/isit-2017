...
lower bounds on source localization accuracy by deriving Cramer-Rao lower
bounds. They also have some preliminary numerical results showing improvement
with increased numbers of sensors. However, this result is also not completely
...
\subsection{Towards a more \textcolor{blue}{physical} sensor model}
...
sensed by this sensor is the $k$-th element of $\v x(\theta)$, given by
\begin{equation}
	x_k(\theta) = \int_{(k-\frac{1}{2})S/m}^{(k+\frac{1}{2})S/m} x(s;\theta) ds
\end{equation}
\textcolor{red}{Also need to talk about noise here - sensor obs. are still iid
because we assume wss noise, and integrating over distinct regions of this
process retains iid nature}
...
between their means:
\begin{equation}
	P_e = \inf_\psi \mathbb P(\psi(Y^n) \neq V) = Q\bigg(\frac{\norm{\v\mu_0 - \v\mu_1}}{2\sigma}\bigg)
\end{equation}
