...
example). \textcolor{blue}{In such a case, one part of the signal $x(t;\theta_0)$ will appear at
the left edge of the domain, and the remaining part will appear as the
repetition from the period $[T, 2T)$, at the right edge of the domain.} Also
note that the two signals $x(t;\theta_0)$ and $x(t;\theta_1)$ overlap at most
once, since $w < T/2$, as stated in section~\ref{sec:signal-model}.

The upper bound in~\eqref{eq:delta-bound} translates into a lower
bound for~\eqref{eq:lb-after-pinsker}:
\begin{align}
	\mathfrak{M}_n(\mathcal{P}, \Phiorho) &\geq \frac{\delta^2}{2} \Bigg[ 1 - \sqrt{\frac{n \cdot 4 \kappa^2 \delta^2}{2\sigma^2} \bigg( \frac{(w + 2\delta) m}{T} + 1\bigg)} \Bigg] \\
	&= \frac{\delta^2}{2} \Bigg[ 1 - \sqrt{\frac{2n \kappa^2 \delta^2}{\sigma^2} \bigg( \frac{(w + 2\delta) m}{T} + 1\bigg)} \Bigg] \label{eq:lb-after-delta-bound}
\end{align}
...
in order to have $\sqrt{\frac{2nm\kappa^2\delta^2 w}{\sigma^2 T}} =
\frac{1}{2}$. This is achieved for $\delta = \sqrt{\frac{\sigma^2T}{8nm\kappa^2
w}}$, so that for $\Phi(\delta) = \delta^2$,
equation~\eqref{eq:lb-after-delta-bound} becomes
\begin{equation}
	\mathfrak{M}_n(\mathcal{P}, \Phiorho) \geq \textcolor{blue}{\frac{1}{32}} \frac{\sigma^2 T}{nm\kappa^2 w}.
\end{equation}
