...
\author{
	\IEEEauthorblockN{
		Praveen Venkatesh (\texttt{vpraveen@cmu.edu}) %(\href{mailto:vpraveen@cmu.edu}{\texttt{vpraveen@cmu.edu}}) %\IEEEauthorrefmark{1}
		and Pulkit Grover (\texttt{pulkit@cmu.edu}) %(\href{mailto:pulkit@cmu.edu}{\texttt{pulkit@cmu.edu}}) %\IEEEauthorrefmark{2}
	}
	\IEEEauthorblockA{
...
%\textcolor{blue}{[NEW VER.1]
%Our recent work~\cite{Grover2016Information} has suggested that EEG source
%localization can benefit greatly from an increase in sensor density. Prior
%work~\cite{Nunez2006Electric} relied on spatial Nyquist rate analyses to
%understand what sensor density is required to reconstruct the EEG signal on the
%\emph{surface of the scalp} to within a given mean squared error.  However,
%that line of research neglected to describe what sensor density is needed to
%recover the signal on the \emph{surface of the brain}; this second question
%also requires an understanding of the noise power spectral density of scalp
%EEG, which has been little-studied. Hence, it is still unknown what the maximum
%imaging resolution attainable with a given number of sensors is, and lower
%bounds providing this understanding do not yet exist.
%}

%\textcolor{green}{[NEW VER.2]
%Three questions naturally arise in such a setting: (1) What algorithms for
%source localization are good? (2) What is the best achievable imaging
%resolution? (3) How does sensor density affect resolution? The first question
%has been addressed by a vast literature on source localization
%algorithms~\cite{Baillet2001Electromagnetic}. The second question calls for
%lower bounds, of which there is as yet very little
%understanding~\cite{Grover2016Fundamental,Mosher1993Error}. The question of
%sensor density has received a lot of attention, but according to us, it still
%remains an open problem. \cite{Nunez2006Electric} uses Nyquist rate arguments
%to understand the number of sensors required to reconstruct the EEG signal on
%the \emph{surface of the scalp} to within a certain MSE. However, they fail to
%address the more relevant question of the number of sensors needed for
%\emph{source localization}, i.e., the number of sensors needed to recover the
%signal on the \emph{surface of the brain}. We show
%in~\cite{Grover2016Information} that this second question begs an understanding
%of the spatial noise power spectral density of scalp EEG, and that under
%appropriate conditions the right algorithms might be able to recover the brain
%signal with much higher fidelity if more sensors were used.
%\cite{Grover2016Information} does not, however, provide rigorous lower bounds
%to substantiate these observations.
%}

%\textcolor{red}{Recent work by Grover~\cite{Grover2016Fundamental} has sought to provide
%    analytical lower bounds on the accuracy of source localization algorithms using an information-theoretic method.
%    but did not provide a lower bound showing if and how this accuracy improves
...
For EEG source localization, we intend to find algorithms that achieve scalings
...
\begin{figure}[t]
	...
	\caption{A numerically generated plot of the lower bound on minimax risk of
		...
		sensors, $m$, following the ``integrator sensor'' model (code available
		online~\cite{FullVersion}). The different curves are for different
		numbers of ``trials'', $n$ (number of i.i.d.\ sensor observations). The
		bound saturates for large $m$, indicating that for a \emph{fixed}
		number of trials, there is decreasing benefit to using more sensors.
		However, the plot suggests that with more trials, the bound saturates
		at increasingly larger numbers of sensors. Thus, there may be benefit
		to increasing the number of sensors when you have more trials.}
	\label{fig:numerical}
\end{figure}
...
\floor*{\frac{S}{c}}$, for sensors of a constant width $c$. Beyond this,
averaging noise over a larger number of trials $n$ may help one achieve a
...
future work. Further, more sophisticated techniques for deriving lower bounds,
such as Fano's method~\cite[Sec.~2.7.1]{Tsybakov2009Introduction} are yet to be
analyzed.
...
%The authors thank Prof.\ Aarti Singh for her useful comments on a preliminary
%draft of this paper.
%The authors would like to thank Philip and Marsha Dowd for their financial
...
In the integrator model of sensors defined in Section~\ref{sec:sensor-model},
recall that the samples are given by equation~\eqref{eq:samples-int-model}.
Consider a sensor located at the point $s_0 \in [0, S)$ and with a width of
$S/m$. That is, we assume each of the $m$ sensors occupies the maximum area
that it can, $1/m$ of the total. As $m$ grows large, the size of each sensor
must shrink in order to accommodate their larger number.

In order to remain consistent with our previous notation, we first define the
\emph{index}, $k$, of the sensor located at $s_0$. (Note that as $m$ increases,
the index $k$ must grow while keeping the position of the sensor fixed, so that
we're seeing how the SNR changes at the same point).
\begin{equation}
	k = \round*{\frac{m s_0}{S}},
\end{equation}
where $\round{z}$ is read as ``round of $z$'', denoting the closest integer to
the point $z \in \mathbb{R}$. Note that as $m$ grows large, $kS/m \to s_0$.

We now compute the approximate SNR of the sensor located at $s_0$ as follows.
The power of the signal is given by the squared absolute value of the samples,
\begin{align}
	\abs{x_k(\theta)}^2 &= \bigg(\int_{(k-\frac{1}{2})S/m}^{(k+\frac{1}{2})S/m} x(s;\theta) ds\bigg)^2 \label{eq:int-sensor-power} \\
	&\overset{(a)}{\approx} \bigg(\frac{S}{m} \; x\Big(\frac{kS}{m}; \theta\Big)\bigg)^2 \\
	&\overset{(b)}{\approx} \frac{S^2}{m^2} \big(x(s_0; \theta)\big)^2 \label{eq:signal-power-k},
\end{align}
where in approximation (a), we have assumed that the signal is roughly constant
in the small interval $\big((k{-}\frac{1}{2})S/m, (k{+}\frac{1}{2})S/m\big)$,
taking the value of the signal at $s = kS/m$. In approximation (b), we make use
of the fact that for sufficiently large $m$, $kS/m \approx s_0$. Note a key
step in the power computation: the integrand in
equation~\eqref{eq:int-sensor-power} is \emph{not} $\abs{x(s; \theta)}^2$,
because the sensor will average out the signal as a part of its sensing
process.

Hence, irrespective of the location of the sensor, $s_0$, the signal power
diminishes as $1/m^2$ since the sensor size \emph{must} diminish at least as
fast as $1/m$, if the number of sensors has to increase to infinity.

The noise variance of the sensor located at $s_0$ is computed similarly:
\begin{align}
...
We can understand how the SNR of the sensor at $s_0$ scales with $m$,
therefore, by taking the ratio of~\eqref{eq:signal-power-k}
and~\eqref{eq:noise-var-k}
\begin{align}
	\text{SNR}(s_0) &= \frac{S^2}{m^2} \big(x(s_0; \theta)\big)^2 \cdot \frac{m}{S\sigma^2} \\
	&= \frac{S}{m\sigma^2} \big(x(s_0; \theta)\big)^2 \propto \frac{1}{m}.
\end{align}
Hence, as the number of sensors $m$ grows to infinity, their sizes must
necessarily reduce at least as fast as $1/m$, and as a result, their SNR also
falls at least as fast as $1/m$, when we view these sensors as integrators.
