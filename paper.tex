...
\textcolor{blue}{certain classes} of impulse responses of the low-pass filter,
when using a unformly distributed sensor array. We show that
...
results in Section~\ref{sec:minimax-lower-bounds}. We conclude in
Section~\ref{sec:discussion} with a discussion on how our results can be
extended to other simple settings.
...
It is also hoped that the techniques presented here will help us derive bounds
in more difficult settings, such as spherical and realistic brain models.
...
$\kappa$. This implies that $\abs{g(u) - g(v)} \leq \kappa \abs{u - v} \;
\forall \;\; u, v \in \Theta$.

The sensors sample the continuous-space shifted impulse response, with some
...
\begin{equation}
	\mathfrak{M}_n(\mathcal{P}, \Phiorho) \geq \frac{1}{32} \frac{\sigma^2 S}{nm\kappa^2 w}.
\end{equation}
...
variance. This is aphysical, since in reality, sensors have a certain width,
and the signal measured by them is the integral of the continuous-space signal
...
uniform prior on the hypotheses, the best possible error probability (over all
tests) is a Q-function~\cite{Qfunction} in the distance between their
...
point at which the bound begins to saturate increases when we use
