...
\begin{align}
	\abs{x_k(\theta)}^2 &= \abs*{\int_{(k-\frac{1}{2})S/m}^{(k+\frac{1}{2})S/m} x(s;\theta) ds.}^2 \\
	&\approx \abs*{\frac{S}{m} x\Big(\frac{kS}{m}; \theta\Big)}^2 \\
	&= \frac{S^2}{m^2} \abs*{x\Big(\frac{kS}{m}; \theta\Big)}^2
\end{align}
...
\begin{align}
	...
	&= \int_{(k-\frac{1}{2}S/m)}^{(k+\frac{1}{2}S/m)} \int_{(k-\frac{1}{2}S/m)}^{(k+\frac{1}{2}S/m)} \delta(s - s') ds ds' \\
	&= \int_{(k-\frac{1}{2}S/m)}^{(k+\frac{1}{2}S/m)} ds \\
	&= \frac{S}{m}
\end{align}
where $R_\epsilon(s, s') = \delta(s - s')$ is the autocorrelation function of
